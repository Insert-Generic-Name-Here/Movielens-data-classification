...
class BSAS:
    ...
    def fit_best(self, data, n_times=20, first_time=True, plot_graph=True):
        var = [name for name in globals() if globals()[name] is X_minimax][0]
        N, l = data.shape
        if (first_time):
            minDist, maxDist = self.__getEuclideanDistances(data, l)
            dists = np.save('min-max-euclidean-distances-%s.npy'%(var), np.array([minDist, maxDist], dtype=np.float))
        else:
            minDist, maxDist = np.load('min-max-euclidean-distances-%s.npy'%(var))
        ...
        if (first_time):
            ...
            np.save('total_clusters-%s.npy'%(var), np.array(total_clusters, dtype=np.int))
            np.save('total_theta-%s.npy'%(var), np.array(total_theta, dtype=np.float))
        else:
